...
for(const [k,v] of Object.entries(wasi_rights))
  wasi_rights[v] = +k;
function wasi_rights_decompose(x){
  const res = [];
...
function wasi_rights_compose(x){
  let res = 0n;
  for(const k of x){
    if(!wasi_rights[k])
...
async function exception_to_errno_a(f, d='EINVAL'){
  try {
    return await f();
  } catch(e) {
    console.error(e);
...
class WASI_Error extends Error {
...
class WASI_Base extends AsyncCreation {
  #fdinfo = [
